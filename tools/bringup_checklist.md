...
| `ATV1` variable DLC | Write `ATV1\r` then `0100\r` to RX, sniffing the CAN bus | The transmitted frame's DLC is `3` (1 PCI byte + 2 payload bytes: service + PID, no padding) rather than `8`. DLC = payload bytes + 1 PCI byte, plus 1 more if `ATCEA` extended addressing is active. **Never run** — confirm with a bus analyzer, since the adapter's own reply gives no direct evidence of the frame's DLC. |
